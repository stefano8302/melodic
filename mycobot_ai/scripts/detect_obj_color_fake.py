...
#      cv2.circle(cv_image, (200,200), 50, 255)
    ...
    cv2.imshow("/camera1/image_raw", cv_image)
    cv2.waitKey(3)
...
class Object_detect(Movement):
    ...
    def __init__(self, camera_x=150, camera_y=-10):
        ...
        # move coordinates
        self.move_coords = [
            [120.1, -141.6, 240.9, -173.34, -8.15, -110.11],  # above the red bucket/cube
            # above the yellow bucket/cube
...
#            self.pub_pump(False, self.Pin)
    ...
    def decide_move(self, x, y, color):
        ...
        if (abs(x - self.cache_x) + abs(y - self.cache_y)) / 2 > 5:  # mm
            ...
            self.cache_x = self.cache_y = 0
            # Adjust the suction position of the suction pump, increase y, move to the left; decrease y, move to the right; increase x, move forward; decrease x, move backward
...
#    cap_num = 0
    ...
    ic = 0
    cap = cv2.VideoCapture(ic, cv2.CAP_V4L)
